...
cleanup() {
    print_status $YELLOW "🔄 Interrupt received, cleaning up..."
    if [ ! -z "$SERVER_PID" ]; then
        kill $SERVER_PID 2>/dev/null
        print_status $BLUE "🧹 Local server stopped"
    fi
    print_status $RED "❌ Pre-commit hook interrupted - commit blocked for safety"
    exit 1
}
