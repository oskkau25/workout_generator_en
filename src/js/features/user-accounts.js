...
export class UserAccount {
  ...
  async register(username, password, profile = {}) {
    try {
      ...
      const normalizedSecurityAnswer =
        typeof profile.securityAnswer === 'string'
          ? profile.securityAnswer.toLowerCase().trim()
          : '';
      const hashedSecurityAnswer = normalizedSecurityAnswer
        ? await this.hashPassword(normalizedSecurityAnswer)
        : '';
  ...
  updateStreak() {
    ...
    if (
      this.currentUser.profile.stats.currentStreak > this.currentUser.profile.stats.longestStreak
    ) {
  ...
  /**
   * Check whether a value looks like a serialized hash payload
   * @param {string} value - Serialized value from storage
   * @returns {boolean} True if this looks like hash metadata
   */
  isSerializedHash(value) {
    if (typeof value !== 'string' || !value.trim()) return false;

    try {
      const parsed = JSON.parse(value);
      return (
        Array.isArray(parsed.hash) &&
  ...
  async resetPassword(username, securityQuestion, securityAnswer, newPassword) {
    try {
      ...
      const normalizedAnswer =
        typeof securityAnswer === 'string' ? securityAnswer.toLowerCase().trim() : '';
      const storedAnswer = user.profile.securityAnswer || '';
      let isValidAnswer = false;

      if (normalizedAnswer && storedAnswer) {
        if (this.isSerializedHash(storedAnswer)) {
          isValidAnswer = await this.verifyPassword(normalizedAnswer, storedAnswer);
        } else {
          // Legacy compatibility: accept historical plaintext and migrate it immediately.
          isValidAnswer = storedAnswer.toLowerCase().trim() === normalizedAnswer;
          if (isValidAnswer) {
            user.profile.securityAnswer = await this.hashPassword(normalizedAnswer);
          }
        }
      }
