 ...
 */
import { trackModeSelected } from './analytics-tracker.js';
...
const REST_SUGGESTIONS = {
  ...
  MOTIVATION: ["You're doing great!", 'Keep pushing forward!', 'Almost there!', "You've got this!"],
  HYDRATION: ['Take a sip of water', 'Stay hydrated!', 'Replenish your fluids'],
};
...
export function initializeEnhancedTimer() {
  ...
  if (workoutState) {
    enhancedTimerState.baseWorkTime =
      Number.isFinite(workoutState.workTime) && workoutState.workTime > 0
        ? workoutState.workTime
        : 45;
    enhancedTimerState.baseRestTime =
      Number.isFinite(workoutState.restTime) && workoutState.restTime > 0
...
function setupTimerModeListeners() {
  ...
  if (modeSelect) {
    ...
    modeSelect.addEventListener('change', (e) => {
      ...
      const workoutState = window.workoutState || {};
      trackModeSelected('timer', e.target.value, {
...
export function playAudioCue(cueType, frequency = 800, duration = 200) {
  ...
  try {
    ...
    gainNode.gain.exponentialRampToValueAtTime(
      0.01,
      enhancedTimerState.audioContext.currentTime + duration / 1000
    );
...
export function getCustomTimerSettings() {
  ...
  return {
    workTime: normalizeTimerValue(
      workTimeInput ? parseInt(workTimeInput.value, 10) : NaN,
      enhancedTimerState.baseWorkTime,
      5,
      300
    ),
    restTime: normalizeTimerValue(
      restTimeInput ? parseInt(restTimeInput.value, 10) : NaN,
      enhancedTimerState.baseRestTime,
      5,
      120
    ),
